...
while (<F>) {
	...
	# print "Obj: $obj_maxdist | $obj_dist | $obj_start_ind | $obj_end_ind | $obj_score | $obj_text\n";
	  ...
	  if ($obj_meta_name ne "" && $obj_entrez_name ne "") {
	   ...
	   $insert_predication_sth->execute($sentence_id, $pmid, $predicate, $subj_id_list, $subj_name_list,$subj_semtype, 			    $obj_id_list, $obj_name_list,$obj_semtype);	   $predication_id = $dbh->{'mysql_insertid'};
