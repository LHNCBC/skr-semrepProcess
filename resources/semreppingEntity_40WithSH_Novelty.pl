...
$insert_sentence_sth = $dbh->prepare_cached('INSERT INTO SENTENCE (PMID,TYPE,NUMBER,SENT_START_INDEX, SENT_END_INDEX, SECTION_HEADER, NORMALIZED_SECTION_HEADER, SENTENCE) VALUES(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE SENTENCE=?') or die "Couldn't prepare statement: " . $dbh->errstr;
$insert_entity_sth = $dbh->prepare_cached('INSERT INTO ENTITY (SENTENCE_ID, PMID, CUI, NAME, SEMTYPE, GENE_ID, GENE_NAME, TEXT, SCORE, START_INDEX, END_INDEX) VALUES(?,?,?,?,?,?,?,?,?,?,?)') or die "Couldn't prepare statement: " . $dbh->errstr;
...
%NoveltyHash = ();
open(N,"<:utf8",$norm_file) or die("Can not open the $norm_file file!");
while (<N>) {
    my($line) = $_;
    chomp($line);
    @line_elements = split(/\|/, $line);
    # print "$line_elements[0]  -> $line_elements[1]\n";
    $NoveltyHash{$line_elements[0]} = $line_elements[1];   
}
...
while (<F>) {
	...
	# retrieve normalized section header
	if(exists($NoveltyHash{$section_header})) {
		$norm_section_header = $NoveltyHash{$section_header};
	} else {
		$norm_section_header = "";
	}
	...
	$insert_sentence_sth->execute($pmid,$senttype,$number,$sent_start_index,$sent_end_index,$section_header,$norm_section_header, $sentence,$sentence) 
      ...
      # Version 4.0 handles the entities as well as predication      
	 ...
	 if($sentence_id) {
		 $insert_entity_sth->execute($sentence_id, $pmid, $cui, $meta_name, $semtype, $entrez_id, $entrez_name, $text, $score, $start_index, $end_index) or die "Couldn't execute statement: " . $sth->errstr;
	 }
       } 
